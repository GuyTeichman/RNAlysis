...
* Fixed a bug where installing DESeq2, limma, or Rsubread's R dependencies (on first use of differential expression or feature counting) could fail with a confusing "could not find function" error after a single transient network/SSL blip while downloading one of many Bioconductor packages -- such a blip left the R package manager silently short one dependency instead of raising an error. Installation now verifies the package actually loads afterward and retries a few times with backoff before giving up.
