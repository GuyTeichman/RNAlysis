...
* The minimum required version of ``aiohttp`` is now 3.12 (from 3.11), guaranteeing the current client's fixes and performance improvements for RNAlysis's asynchronous web requests (such as Ensembl ortholog mapping and the other ``aiohttp``-based fetches in ``utils/io.py``). Analysis results are unchanged.
* The minimum required version of the optional ``numba`` package (used by the randomization tests) is now 0.63 (from 0.61). Fresh installs already resolved a current ``numba``; this floor protects existing environments — ``numba`` 0.61/0.62 cannot run against current NumPy versions or Python 3.14, so an old pre-installed ``numba`` could hold NumPy back or break the ``randomization`` extra after a Python upgrade. Results are unchanged.
...
* Fixed a bug where exporting a Pipeline crashed with a cryptic ``RepresenterError: cannot represent an object`` if any of its function parameters was a NumPy value (for example a threshold taken straight from a results table) or a ``pathlib.Path``. Such values are now converted to their plain equivalents when the Pipeline is saved, and a value that genuinely cannot be saved to a Pipeline file now produces a clear message naming the function and the parameter at fault. Note that YAML has no tuple type: a tuple nested inside a parameter is saved — and re-loaded — as a list. This is now documented in ``export_pipeline`` and ``import_pipeline``.
