...
Changed
*******
* Recolored the GUI's boolean toggle switch: the off/False state is now a neutral inactive gray instead of a red-orange that read as an error, and the on/True state is now the RNAlysis brand red (from the logo palette) instead of green. Because the off state is a neutral gray, the red/gray pairing stays off the red-green confusion axis, and the switch still encodes state redundantly through knob position and a "True"/"False" text label, so it remains distinguishable without relying on color.
