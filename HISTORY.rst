...
* When the new ``'log'`` transform is chosen for CLICOM ensemble clustering (``CountFilter.split_clicom``), its cluster-expression plots now display log-transformed values, matching the transform the clustering itself was run with. Cluster assignments and every returned table are unaffected, and the cluster-expression plots drawn for the other transforms are unchanged.
