...
* Fixed a bug where enrichment analyses (GO, KEGG, and user-defined attributes) could crash with a confusing ``not enough values to unpack`` error when the enrichment gene set ended up empty — for example when a web service temporarily mapped none of the genes to the required ID type, or when every gene was filtered out. Such analyses now finish gracefully with an empty result instead of crashing.
