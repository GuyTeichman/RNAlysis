...
@pytest.mark.parametrize('import_statement', ['import rnalysis.filtering', 'from rnalysis import filtering',
                                              'import rnalysis.enrichment', 'from rnalysis import enrichment'])
def test_importing_the_api_does_not_import_heavy_packages(import_statement):
    ...
    assert result['loaded_submodules'] == [], \
        f"{import_statement!r} eagerly imported {result['loaded_submodules']}"
