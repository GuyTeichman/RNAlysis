...
@pytest.mark.parametrize(
    'gtf_path,feature_type,len_method,truth',
    [
        (
            'tests/test_files/test_gtf_wormbase.gtf',
            'gene',
            'mean',
            {'WBGene00197333': 150, 'WBGene00198386': 150, 'WBGene00015153': 1177.5, 'WBGene00002061': 6137 / 6},
        ),
        (
            'tests/test_files/test_gtf_wormbase.gtf',
            'gene',
            'median',
            {'WBGene00197333': 150, 'WBGene00198386': 150, 'WBGene00015153': 1177.5, 'WBGene00002061': 1021.5},
        ),
        (
            'tests/test_files/test_gtf_wormbase.gtf',
            'gene',
        ...
        (
            ...
            'gene',
            'merged_exons',
            {'WBGene00197333': 150, 'WBGene00198386': 150, 'WBGene00015153': 1327, 'WBGene00002061': 1108},
        ),
        (
            'tests/test_files/test_gtf_wormbase.gtf',
            'gene',
            'min',
            {'WBGene00197333': 150, 'WBGene00198386': 150, 'WBGene00015153': 1028, 'WBGene00002061': 940},
        ),
        (
            'tests/test_files/test_gtf_wormbase.gtf',
            'transcript',
        ...
        (
            'tests/test_files/test_gff_wormbase.gff3',
            'gene',
            'geometric_mean',
            {'WBGene00015153': 1167.9708900482055, 'WBGene00002061': 1019.8567195006932},
        ),
        ('tests/test_files/test_gff_wormbase.gff3', 'gene', 'max', {'WBGene00015153': 1327, 'WBGene00002061': 1108}),
        (
            'tests/test_files/test_gff_wormbase.gff3',
            'gene',
            'merged_exons',
            {'WBGene00015153': 1327, 'WBGene00002061': 1108},
        ),
    ],
)
...
@pytest.mark.parametrize(
    'use_name,use_version,split_ids,truth',
    [
        ...
        (
            ...
            False,
            {
                'ENST00000000001.2': 'ENSG00000000001.3',
                'ENST00000000002.1': 'ENSG00000000001.3',
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            False,
            False,
            {'ENST00000000001.2': 'GENEA', 'ENST00000000002.1': 'GENEA', 'ENST00000000003.4': 'GENEB'},
        ),
        (True, False, True, {'ENST00000000001': 'GENEA', 'ENST00000000002': 'GENEA', 'ENST00000000003': 'GENEB'}),
        (True, True, False, {'ENST00000000001.2': 'GENEA', 'ENST00000000002.1': 'GENEA', 'ENST00000000003.4': 'GENEB'}),
...
def test_map_transcripts_to_genes_wormbase_split_ids_collapse():
    ...
    assert map_transcripts_to_genes('tests/test_files/test_gtf_wormbase.gtf') == {
        'cTel3X': 'WBGene00197333',
        'B0348': 'WBGene00015153',
    }
    # With split_ids=False every transcript is preserved (the intended mapping for this file).
    assert map_transcripts_to_genes(
        'tests/test_files/test_gtf_wormbase.gtf', use_name=False, use_version=False, split_ids=False
    ) == {
        'cTel3X.2': 'WBGene00197333',
        'cTel3X.3': 'WBGene00198386',
        'B0348.5a': 'WBGene00015153',
        'B0348.5b': 'WBGene00015153',
        'B0348.6b.2': 'WBGene00002061',
        'B0348.6a.2': 'WBGene00002061',
        'B0348.6b.1': 'WBGene00002061',
        'B0348.6c.2': 'WBGene00002061',
        'B0348.6a.1': 'WBGene00002061',
        'B0348.6c.1': 'WBGene00002061',
    }
...
@pytest.mark.parametrize(
    'gtf_path,gene_keys',
    [(ENSEMBL_GTF, ('ENSG00000000001', 'ENSG00000000002')), (GENCODE_GTF, ('ENSG00000000001.3', 'ENSG00000000002.1'))],
)
@pytest.mark.parametrize(
    'method,gene_a_len',
    [
        ...
        ('max', 200),
        ('min', 100),
        ('geometric_mean', 141.42135623730945),
        ('merged_exons', 200),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ('max', 200),
        ('min', 150),
        ('geometric_mean', 173.2050807568876),
        ('merged_exons', 250),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        (
            ...
            False,
            False,
            {'ENST00000000001': 'protein_coding', 'ENST00000000002': 'protein_coding', 'ENST00000000003': 'lincRNA'},
        ),
        (
            False,
            True,
            False,
            {
                'ENST00000000001.2': 'protein_coding',
                'ENST00000000002.1': 'protein_coding',
...
@pytest.mark.parametrize(
    'feature_type,attribute,split_ids,truth',
    [
...
def test_map_transcripts_to_genes_gff3_multiparent(use_version, split_ids):
    ...
    assert map_transcripts_to_genes(MULTIPARENT_GFF3, use_name=False, use_version=use_version, split_ids=split_ids) == {
        'ENST1': 'ENSG1',
        'ENST2': 'ENSG1',
    }
...
def test_map_gene_to_attr_gff3_transcript():
    ...
    assert map_gene_to_attr(MULTIPARENT_GFF3, 'biotype', 'transcript', False, False, False) == {
        'ENST1': 'protein_coding',
        'ENST2': 'protein_coding',
    }
...
def test_map_gene_to_attr_biotype_resolution_stays_within_feature_level(tmp_path):
    ...
    assert map_gene_to_attr(
        _gene_type_with_transcript_biotype_gtf(tmp_path), 'gene_biotype', 'gene', False, False, False
    ) == {'G1': 'lincRNA'}
...
ATTRIBUTES_GFF3 = (
    'tests/test_files/test_gff3_attributes.gff3'  # same content, with SO-term ID prefixes (gene:/transcript:)
)
