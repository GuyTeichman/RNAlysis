...
design <- model.matrix(~ condition + replicate + covariate1 + covariate2 + factor1 + factor2, design_matrix)
...
cov_res <- topTable(bayes_fit, n=Inf, coef="covariate1")
cov_res_ordered <- cov_res[order(cov_res$adj.P.Val),]
write.csv(as.data.frame(cov_res_ordered),file="*PLACEHOLDERPATH*/LimmaVoom_covariate1_covariate.csv")
