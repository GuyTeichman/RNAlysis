...
from tests import __attr_ref__, __biotype_ref__, is_ensembl_available, is_uniprot_available
...
def test_featureset_intersection():
    ...
    truth = {'WBGene00017419', 'WBGene00016520', 'WBGene00017225', 'WBGene00044200', 'WBGene00206390', 'WBGene00022523'}
    up = FeatureSet(up_feature_set)
    other_set = FeatureSet(other)
...
def test_enrichment_randomization_reproducibility(protein_coding_set):
    ...
    def run(seed):
        res = en.user_defined_enrichment(
            protein_coding_set,
            attrs,
            statistical_test='randomization',
            attr_ref_path=__attr_ref__,
            randomization_reps=1000,
            random_seed=seed,
...
def test_enrich_hypergeometric_pvalues(all_genes_set):
    ...
    en = FeatureSet(gene_set=genes, set_name='test_set')
    res = en.user_defined_enrichment(
        all_genes_set, attrs, statistical_test='hypergeometric', attr_ref_path=__attr_ref__
    )
    plt.close('all')
    _enrichment_validity(res, truth)
...
@pytest.mark.parametrize(
    's',
    [RankedSet([], 'name'), RankedSet(['1', '2', '3', '4', '5']), RankedSet(['a', 'c', 'b', 'e', 'd'], 'othername')],
)
...
def test_enrich_non_categorical_api(protein_coding_set):
    ...
    res = s.non_categorical_enrichment(
        protein_coding_set,
        ['attr3', 'attr2', 'attr4'],
...
def test_enrich_non_categorial_parametric_test():
    ...
    res_param = fs.non_categorical_enrichment(
        background, 'all', alpha=0.15, parametric_test=True, attr_ref_path=ref_table
    )
    ...
    assert np.allclose(
        res_param.select(pl.col(isclose_cols)), res_param_truth.select(pl.col(isclose_cols)), atol=0.0, rtol=0.01
    )
...
def test_enrich_non_categorial_nonparametric_test():
    ...
    isclose_cols = ['obs', 'exp', 'pval', 'padj']
    assert np.allclose(
        res_nonparam.select(pl.col(isclose_cols)), res_nonparam_truth.select(pl.col(isclose_cols)), atol=0.0, rtol=0.01
    )
    plt.close('all')
...
def test_enrich_non_categorical_nan_values():
    ...
    for df, df_truth in zip((res_nonparam, res_param), (truth, truth_param)):
        ...
        assert df.select(pl.col(['samples', 'significant'])).equals(df_truth.select(pl.col(['samples', 'significant'])))
        assert np.allclose(
            df.select(pl.col(['obs', 'exp'])), df_truth.select(pl.col(['obs', 'exp'])), rtol=0.01, equal_nan=True
        )
        assert np.allclose(
            df.select(pl.col(['pval', 'padj'])),
            df_truth.select(pl.col(['pval', 'padj'])),
            atol=0,
            rtol=0.02,
            equal_nan=True,
        )
...
@pytest.mark.skipif(not UNIPROT_AVAILABLE, reason='UniProt REST API is not available at the moment')
@pytest.mark.parametrize(
    'organism,statistical_test,propagate_annotations,kwargs',
    [(6239, 'fisher', 'elim', {}), ('caenorhabditis elegans', 'randomization', 'no', dict(randomization_reps=100))],
)
    ...
    upset_plot(
        {
            'obj': 'attribute1',
            'obj2': 'attribute2',
            'obj3': {'WBGene00000001', 'WBGene00001234'},
            'obj4': 'attribute3',
            'obj5': 'attribute4',
        },
...
def test_venn_diagram_api():
    ...
    venn_diagram(
        {'obj': {'0', '1', '2'}, 'obj2': FeatureSet({'1', '2', '3'}), 'obj3': RankedSet(['5', '3', '0'])},
        title='title',
        set_colors=('black', 'purple', 'yellow'),
        transparency=0.2,
        linecolor='grey',
        linestyle='dashed',
        linewidth=1,
    ...
    venn_diagram(
        {'obj': 'attribute1', 'obj2': 'attribute2', 'obj3': {'WBGene00000001', 'WBGene00001234'}},
        attr_ref_table_path=__attr_ref__,
        add_outline=False,
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            {
                'set1': RankedSet(['one', 'two']),
                'set2': Filter('tests/test_files/test_fetch_sets_table.csv'),
                'set3': 'attribute2',
            },
            {
                'set1': {'one', 'two'},
...
@pytest.mark.skipif(not UNIPROT_AVAILABLE, reason='UniProt REST API is not available at the moment')
@pytest.mark.parametrize(
    'organism,statistical_test,kwargs',
    [(6239, 'fisher', {}), ('caenorhabditis elegans', 'randomization', dict(randomization_reps=100))],
)
def test_kegg_enrichment_api(organism, statistical_test, kwargs, protein_coding_set):
    genes = {
        ...
        'WBGene00011910',
        'WBGene00268195',
        'WBGene00000833WBGene00007150',
        'WBGene00016995',
    }
    en = FeatureSet(gene_set=genes, set_name='test_set')
    _ = en.kegg_enrichment(
        protein_coding_set, organism=organism, gene_id_type='WormBase', statistical_test=statistical_test, **kwargs
    )
    plt.close('all')
...
@pytest.mark.parametrize(
    'map_to,map_from,remove_unmapped_genes,expected,expected_name',
    [
        (
            'UniProtKB AC/ID',
            'WormBase',
            False,
            'tests/test_files/counted_translated_with_unmapped.csv',
        ...
        (
            ...
            'auto',
            True,
            'tests/test_files/counted_translated_remove_unmapped.csv',
...
def test_find_paralogs_ensembl(mock_mapper, filter_percent_identity):
    ...
    featureset = FeatureSet(Filter('tests/test_files/test_map_orthologs.csv'))
    result = featureset.find_paralogs_ensembl(
...
def test_map_orthologs_panther(mock_mapper, non_unique_mode, filter_least_diverged, remove_unmapped, ranked_set):
    ...
    if ranked_set:
        ...
        featureset = FeatureSet(Filter('tests/test_files/test_map_orthologs.csv'))
    one2one, one2many = featureset.map_orthologs_panther(
        'Homo sapiens',
...
def test_map_orthologs_ensembl(mock_mapper, non_unique_mode, filter_percent_identity, remove_unmapped, ranked_set):
    ...
    one2one, one2many = featureset.map_orthologs_ensembl(
        'Homo sapiens',
        'caenorhabditis elegans',
        gene_id_type='UniProtKB',
        inplace=False,
...
def test_map_orthologs_phylomedb(
    mock_mapper, non_unique_mode, filter_consistency_score, remove_unmapped, threshold, ranked_set
):
    if remove_unmapped:
        filter_obj_path = 'tests/test_files/test_map_orthologs_remove_unmapped_truth.csv'
    else:
    ...
    if ranked_set:
        ...
        featureset = FeatureSet(Filter('tests/test_files/test_map_orthologs.csv'))
    one2one, one2many = featureset.map_orthologs_phylomedb(
        'Homo sapiens',
        'caenorhabditis elegans',
        gene_id_type='UniProtKB',
        inplace=False,
...
def test_map_orthologs_orthoinspector(mock_mapper, non_unique_mode, remove_unmapped, ranked_set):
    ...
    if ranked_set:
        ...
        featureset = FeatureSet(Filter('tests/test_files/test_map_orthologs.csv'))
    one2one, one2many = featureset.map_orthologs_orthoinspector(
        'Homo sapiens',
        'caenorhabditis elegans',
        gene_id_type='UniProtKB',
        inplace=False,
