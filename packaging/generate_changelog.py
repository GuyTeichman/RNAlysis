...
def get_change_log_for(version: Union[str, Literal['latest']] = 'latest', history_path: Union[str, Path, None] = None):
    ...
    if version == 'latest':
        ...
        if newest is not None and re.fullmatch(r'\d{4}-\d{2}-\d{2}', newest.group(2)) is None:
            raise ValueError(
                f"The top HISTORY.rst section ('{newest.group(1)} ({newest.group(2)})') is "
