...
4. Tag `V<version>` and push it — this triggers `.github/workflows/pyinstaller.yml`, which builds
...
5. Publish to PyPI (`make dist && twine upload dist/*` — this step is manual, despite what
   `CONTRIBUTING.rst` says), then merge `master` back into `development` so it doesn't drift.

The `bumpversion` push and the Pyinstaller workflow's changelog/checksum auto-commit are the two
narrow, deliberate exceptions to "never commit directly to `master`" — both need the maintainer's
own admin push access (`master`'s branch protection has `enforce_admins: false`), not just any
contributor's. See the skill for why, and for the full post-release verification checklist.
